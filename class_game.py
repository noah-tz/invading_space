...
class Game:
	def __init__(self, level, life, score):
		...
		self.score = score
		self.stay = True
		self.image_game_over = IMG_GAME_OVER
		self.image_you_winner = IMG_YOU_WINNER
		self.ship = Ship((WIDTH - 100) // 2, int(HEIGHT * 0.8))

	def background_music(self):
		explosion_sound = pg.mixer.Sound(SOUND_GAME)
		pg.mixer.Sound.play(explosion_sound)

	def shot_sound(self):
		pg.mixer.init()
		pg.mixer.music.load(SOUND_SHOT)
		pg.mixer.music.play()

	def text(self, size, vec, txt):
		font = pg.font.Font('freesansbold.ttf', size)
		text = font.render(txt, True, RED, WHITE)
		...
		for event in pg.event.get():
			...
			if event.type == pg.KEYDOWN:
				if event.key == pg.K_SPACE:
					self.shot_sound()
					self.shots_ship.add(Shot_ship(self.ship.rect.x + (WIDTH_SHIP / 2) - (WIDTH_SHOT / 2), self.ship.rect.y))
					self.score -= 1
		keys = pg.key.get_pressed()
		if keys[pg.K_LEFT] and self.ship.rect.x > 10:
			self.ship.move_ship(- SPEED_MOVE_SHIP)
		elif keys[pg.K_RIGHT] and self.ship.rect.x < (WIDTH - 57):
			self.ship.move_ship(SPEED_MOVE_SHIP)
	...
	def move_invaders(self, invaders):
		if len(self.invaders_group) > 0:
			...
			if (x_min <= 0) and (self.revers == True):
				self.invaders_group.update(0, self.level * 0.2 + SPEED_DOWN_INVADERS)
				self.revers = False
			if (x_max >= (HEIGHT * 1.28)) and (self.revers == False):
				self.revers = True
				self.invaders_group.update(0, self.level * 0.2 + SPEED_DOWN_INVADERS)
			if self.revers:
				self.invaders_group.update(- SPEED_MOVE_INVADERS, 0)
			else:
				self.invaders_group.update(SPEED_MOVE_INVADERS, 0)

	def shot_invaders(self):
		shoting = random.randint(0, PULSE_SHOTS_INVADERS + int(self.level * 0.1 + 1))
		create_joker = random.randint(0, PULSE_CREATE_JOKER)
		if shoting == PULSE_SHOTS_INVADERS:
			inverder_shoting = random.choice(self.invaders_group.sprites())
			self.shots_invaders.add(Shot_invaders(inverder_shoting.rect.x + (WIDTH_INVADER / 2) - (WIDTH_SHOT / 2), inverder_shoting.rect.y + (HEIGHT_INVADER)))
		if create_joker == PULSE_CREATE_JOKER:
	...
	def blit_object(self):
		...
		self.text(25, (HEIGHT * LOCATION_TEXT_GAME_X, WIDTH * LOCATION_TEXT_GAME_Y ), f"your score is {self.score}, your life is {self.life}. level: {self.level }")
			...
			if not len(self.invaders_group) > 0:
				...
				for event in pg.event.get():
					try:
						...
						a = 1
				self.text(40, (HEIGHT * 0.7, WIDTH * 0.6), f"your score is {self.score}")
				self.text(40, (HEIGHT * 0.7, WIDTH * 0.1), f"press key-down for up level")
			else:
				...
				self.text(40, (HEIGHT * 0.7, WIDTH * 0.6), f"your score is {self.score}")
				self.text(40, (HEIGHT * 0.7, WIDTH * 0.1), f"press key-down for play again")
			pg.display.flip()
			self.clock.tick(5)
